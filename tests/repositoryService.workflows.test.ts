...
import {
  cleanupTempRoots, cloneRemote, createRemoteBackedRepository, createService, createTempRepository,
  git, tempRoots
...
describe('RepositoryService', () => {
  ...
  it('force-pushes rewritten branch history with confirmation', async () => {
    const { repoPath, remotePath } = createRemoteBackedRepository()
    const service = createService()

    await service.branches.publishBranch({ repoPath })
    writeFileSync(path.join(repoPath, 'tracked.txt'), 'first pushed\n')
    git(repoPath, ['add', 'tracked.txt'])
    git(repoPath, ['commit', '-m', 'First pushed update'])
    await service.push(repoPath)

    git(repoPath, ['reset', '--hard', 'HEAD~1'])
    writeFileSync(path.join(repoPath, 'tracked.txt'), 'rewritten\n')
    git(repoPath, ['add', 'tracked.txt'])
    git(repoPath, ['commit', '-m', 'Rewrite pushed update'])

    await expect(service.forcePush({ repoPath, confirmed: false })).rejects.toMatchObject({ code: 'confirmation_required' })

    const snapshot = await service.forcePush({ repoPath, confirmed: true })

    expect(snapshot.summary.ahead).toBe(0)
    expect(git(remotePath, ['log', '-1', '--pretty=%s', 'main'])).toBe('Rewrite pushed update')
  })
