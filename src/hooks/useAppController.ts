...
export function useAppController() {
  ...
  const {
    ...
    dailyReviewLoading, contributorStats, loadContributorStats, runDailyReview, copyDailyReviewMarkdown
  } = useDailyReview({ api, currentRepoPath, setNotice, setError, copyToClipboard })

  // Refresh the contributor leaderboard when the Reports (daily) view is open.
  // In All-repositories mode it aggregates across every recent repository.
  useEffect(() => {
    if (viewMode !== 'daily') return
    void loadContributorStats(allReposMode ? undefined : currentRepoPath)
  }, [snapshot?.summary.rootPath, viewMode, allReposMode])
